...
export const POSTS = [
  {
    ...
    body: [
      ...
      { type: 'code', text: "// email worker\napp.post('/send-email', async (req, res) => {\n  const { scheduleId, recipient, template } = req.body;\n\n  const alreadySent = await sentTable.has(scheduleId);\n  if (alreadySent) {\n    return res.status(200).json({ status: 'already_sent' });\n  }\n\n  await sendEmail({ recipient, template });\n  await sentTable.insert(scheduleId);\n\n  return res.status(200).json({ status: 'sent' });\n});" },
